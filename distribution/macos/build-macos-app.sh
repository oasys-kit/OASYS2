...
Options:
    ...
    --pip-arg  ARG
        ...
        If not supplied then by default the latest PyPi published Oasys2 is
        used.
...
Examples
    build-macos-app.sh ~/Applications/Oasys2.app
        Build the application using the latest published version on pypi

    build-macos-app.sh --pip-arg={Oasys2==0.0.1,PyQt5} ~/Applications/Oasys2.app
        Build the application using the specified Oasys version

    build-macos-app.sh --pip-arg=path-tolocal-checkout ~/Applications/Oasys2-Dev.app
        Build the application using a local source checkout

    build-macos-app.sh --pip-arg={-e,path-tolocal-checkout}  ~/Applications/Oasys2-Dev.app
        Build the application and install oasys in editable mode

    buils-macos-app.sh --pip-arg={-r,requirements.txt} /Applications/Oasys2.app
...
exec "${DIR}"/PythonApp -m oasys2.canvas "$@"
EOF
chmod +x "${APPDIR}"/Contents/MacOS/Oasys2
...
#    trap cleanup EXIT
#    cd "${tempdir}"
#    "${PYTHON}" -m pip install --no-cache-dir --no-index oasys2
#"${PYTHON}" -m oasys2.canvas --help > /dev/null
#)
